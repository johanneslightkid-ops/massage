...
export const onRequest: PagesFunction<Env> = async (context) => {
  ...
  try {
    ...
    if (path === 'admin/settings' && method === 'PUT') {
      ...
      const current = await readContent(kv)
      await writeContent(kv, { ...current, site: { ...current.site, ...body } as SiteContent['site'] })
      return json({ ok: true })
    }
    ...
    if (path.startsWith('admin/collection/') && method === 'PUT') {
      ...
      const current = await readContent(kv)
      await writeContent(kv, { ...current, [key]: body } as SiteContent)
    ...
    if (path === 'admin/reset' && method === 'POST') {
      ...
      if (isCollectionKey(section)) {
        const current = await readContent(kv)
        await writeContent(kv, { ...current, [section]: structuredClone(seedContent[section]) } as SiteContent)
        return json({ ok: true, section })
      }
