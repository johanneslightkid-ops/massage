    ...
    <AnimatePresence>
      {visible && (
        <motion.a
          ...
          aria-label={t('action.chat_whatsapp')}
          initial={{ opacity: 0, scale: 0.6, y: 20 }}
          animate={{ opacity: 1, scale: 1, y: 0 }}
          exit={{ opacity: 0, scale: 0.6, y: 20 }}
          transition={{ type: 'spring', stiffness: 340, damping: 24 }}
          className="fixed right-4 bottom-[calc(env(safe-area-inset-bottom,0px)+5.9rem)] z-50 grid size-14 place-items-center rounded-full bg-[#25D366] text-[#062e17] shadow-lift lg:right-8 lg:bottom-8 lg:size-16"
        >
